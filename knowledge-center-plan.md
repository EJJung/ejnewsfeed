...
**Capture half (3d–3e), text-first.** Split again into a session spec (the companion + transcript) and a write-back spec, so the write-back extraction is designed against a real transcript. The **text companion session ✅ COMPLETE (2026-08-24, backend live-verified)**; write-back (3e) is the remaining spec.
...
**3e. Write-back (the flywheel).** ⏸️ next spec. After each session, Claude processes the `session_messages` transcript: decisions made → `decisions`; hypotheses formed/revised → `hypotheses`; new open questions → `open_questions`; meeting summary stored on the meeting record (`meetings.summary`, reserved for this). Behind a propose→approve→commit gate (EJ reviews extracted items before they enter the knowledge layer). The next pack assembly reads all of it. To be brainstormed now that a real transcript exists.
