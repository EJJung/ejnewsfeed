...
### Phase 4 — Compounding extras — ✅ COMPLETE (2026-08-27)

All three directions shipped, each its own spec/plan/build cycle, subagent-driven with per-task + whole-branch review and live E2E:

- ✅ **Knowledge layer as MCP/API** (PR #8) — a local stdio MCP server (`knowledge_mcp/`, Python) exposing the knowledge layer to Claude Code skills as 7 read-only query tools (`search_insights`, `get_contested`, `knowledge_query`, `get_insight_sources`, `get_open_questions`, `get_decisions`, `get_hypotheses`). service_role key, read-only by construction; registered via repo-root `.mcp.json`. This is the plan's north-star: any skill can now ask "what do we currently believe about X, and what's contested?". Spec `docs/superpowers/specs/2026-08-26-knowledge-mcp-design.md`.
- ✅ **Insight-graph visualization** (PR #9) — a dashboard "Insight Graph" view: insights as a co-citation graph (`react-force-graph-2d`), nodes colored by domain and sized by source count, edges = shared source articles, with a domain filter, candidate toggle, and click-for-side-panel. Pure `buildGraph` computes edges client-side. Honest to today's data (renders as co-citation; 0 contested/contradicting yet — edge model ready to absorb them). Spec `docs/superpowers/specs/2026-08-27-insight-graph-design.md`.
- ✅ **Engagement-based recommendation layer** (PR #10) — two parts: (A) wired the previously-unused `logInteraction` into `opened`/`saved`/`read_full` so `user_interactions` finally accrues (verified live: a click writes real rows), and (B) a "Recommended" view ranking articles by a blended `0.35*impact + 0.20*recency + 0.25*category-affinity + 0.15*sources-an-active-insight + 0.05*active-domain-category` score with reason tags. Cold-start carried by the knowledge boost; sharpens as engagement accrues. The live E2E caught a white-screen crash the static review missed (a `source:{name}` object rendered as a React child), fixed before merge. Spec `docs/superpowers/specs/2026-08-27-recommendation-layer-design.md`.
